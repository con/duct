...
User
 43                 monitor_process(process.pid, elapsed_time, report_interval, process_data)

here we are using the process pid, but instead we should use the session id to retrieve a list of pids to monitor
ChatGPT

To monitor all processes within a session rather than just a single process identified by its PID, you need to modify the script to work with session IDs and retrieve a list of PIDs within that session. This approach allows you to monitor not only the main process but also all its child processes.

In Unix-like operating systems, each process belongs to a session, and each session can contain multiple processes. To implement this functionality, you need to:

    Retrieve the session ID of the process started by your wrapper.
    Find all processes that belong to this session.
    Monitor all these processes as part of your reporting.

Here's how you can modify your script to incorporate these changes:
Modifications to the Script

First, let's modify the monitor_process function to handle a session ID and find all PIDs in that session.
...
Key Points
    ...
    Process Session: The session ID is obtained using os.getsid(process.pid), which retrieves the session ID of the lead process.
    /proc File System: The script now reads from /proc/<pid>/stat to find all processes in the same session. This is specific to Linux-like systems.
    Monitoring All Processes: It now monitors all processes in the session, not just the initial process.

This modification allows you to comprehensively monitor all subprocesses spawned by the initial command, providing more detailed insights into the behavior of complex commands that spawn multiple processes.
